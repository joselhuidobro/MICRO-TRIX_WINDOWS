...
# Estructuras binarios: 3 floats (pitch/roll/yaw) + 4 int16 (m1..m4) + opcional uint32 (t_us)
...
def _udp_loop(udp_port: int, publish_update):
...
def start_udp_listener(udp_port: int, publish_update):
    """Lanza el hilo del listener UDP y lo deja en daemon."""
    th = threading.Thread(target=_udp_loop, args=(udp_port, publish_update), daemon=True)
    th.start()
    return th
